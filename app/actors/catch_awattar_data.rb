...
class CatchAwattarData < Actor
  input :starts_at
  input :ends_at, default: Time.now

  def call
    awattar_data['data'].each do |awattar_record|
      starts_at = Time.at(awattar_record['start_timestamp'] / 1000).strftime('%Y-%m-%d %H:%M:%S')
      ends_at = Time.at(awattar_record['end_timestamp'] / 1000).strftime('%Y-%m-%d %H:%M:%S')
